...
FSMEDITOR_API bool UFSMTransitionNode::CreateConnections(UFSMGraphNode* PreviousState, UFSMGraphNode* NextState, FString InTransName)
{
	//此处是做名字校验，看传入的名字是否合规，不符合则"_1"加后缀。
	TSharedPtr<INameValidatorInterface> NameValidator = MakeNameValidator();/*FNameValidatorFactory::MakeValidator(this);*/
	FString TransName = InTransName;
	NameValidator->FindValidString(TransName);

	//设置Transition变量名字。
	SetTransitionName(FName(*TransName));
	...
	if (!FSM->AddRule(GetTransitionName(), FromNode->GetNodeName(), TargetNode->GetNodeName()))
	{
		return false;
	}

	//下面都是UEdGraphSchema::TryCreateConnection()函数内部的实现方法
	// Previous to this
	GetInputPin()->Modify();
	...
	this->GetOrAddSlot(ENodeZone::Center)
		...
		[
			...
			[
				...
				//.BorderBackgroundColor(this, &SFSMTransitionNode::GetTransitionColor)	//上面用了黑色的底图，导致无法设置悬停。
				.ColorAndOpacity(this, &SFSMTransitionNode::GetTransitionColor)		//黑色底图，所以我们从其Content包裹的文字去设置悬停颜色改变。
				.HAlign(HAlign_Center)
				.VAlign(VAlign_Center)
...
void SFSMTransitionNode::MoveTo(const FVector2D & NewPosition, FNodeSet & NodeFilter, bool bMarkDirty)
{
...
FLinearColor SFSMTransitionNode::GetTransitionColor() const
{
	return StaticGetTransitionNodeColor(IsHovered());
}
...
FLinearColor SFSMTransitionNode::StaticGetTransitionNodeColor(bool bIsHovered)
{
	const FLinearColor HoverColor(0.724f, 0.256f, 0.0f, 1.0f);
	FLinearColor BaseColor(1.f, 1.f, 1.f, 1.f);
