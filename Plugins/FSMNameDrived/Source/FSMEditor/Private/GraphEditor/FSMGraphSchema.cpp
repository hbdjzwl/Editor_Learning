...
//注意: 此处只是增加了多个选项的UI，实际触发的规则还是在 FFSMAssetEditor::CreateGraphEditorWidget()中。最后AdditionalCommands(GraphEditorCommands)
void UFSMGraphSchema::GetContextMenuActions(UToolMenu* Menu, UGraphNodeContextMenuContext* Context) const
{
	check(Context);

	//改函数因为版本不同，修改过，对节点右键的展开的时候会触发。
	if (Context->Node != NULL)
	{
		FToolMenuSection& ToolMenuSection = Menu->AddSection("FSMNodeActions", LOCTEXT("NodeActionsMenuHeader", "Node Actions"));

		//应该是将选项行为注册到右键上。
		ToolMenuSection.AddMenuEntry(FGenericCommands::Get().Delete);
		ToolMenuSection.AddMenuEntry(FGraphEditorCommands::Get().ReconstructNodes);
		ToolMenuSection.AddMenuEntry(FGraphEditorCommands::Get().BreakNodeLinks);

		if(Context->Node->bCanRenameNode)
		{
			ToolMenuSection.AddMenuEntry(FGenericCommands::Get().Rename);
		}
	}
...
bool UFSMGraphSchema::CreateAutomaticConversionNodeAndConnections(UEdGraphPin * PinA, UEdGraphPin * PinB) const
{
		...
		&& (NodeB->GetInputPin() != NULL) && (NodeB->GetOutputPin() != NULL))
	{
		...
		{
			return TransitionNode->CreateConnections(NodeA, NodeB, TEXT("Transition"));
		}
		else
		{
			return TransitionNode->CreateConnections(NodeB, NodeA, TEXT("Transition"));
		}
		/*
