...
bool UFSM::HasRuleByState(const FName & FromState, const FName & RuleName) const
{
...
bool UFSM::HasRule(const FName& RuleName) const
{
	TArray<FName> AllStateNode;
	StateMachine.CurrState_Mapping_TransRule.GetKeys(AllStateNode);
	for (const FName& State : AllStateNode)
	{
		if (HasRuleByState(State, RuleName))
		{
			return true;
		}
	}
...
bool UFSM::Modify(bool bAlwaysMarkDirty /*= true*/)
{
